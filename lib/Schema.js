...
var Schema = function () {
    ...
    return {
        ...
        putOne       : function (schema, example) {
            ...
            if (primaryKeyField) {
                if (primaryKeyField.autoIncrement) {
                    if (parseInt('' + example[primaryKeyField.name], 10)) {
                        var updates = [];
                        decaf.each(schema.fields, function (field) {
        ...
        getFromTable : function (name) {
            ...
            var fields = [];
            /*
             select column_name, data_type, column_default,character_maximum_length from INFORMATION_SCHEMA.COLUMNS where table_name = '<name of table>';
             */
            ...
            decaf.each(rows, function (row) {
                var unsigned = row['data_type'].indexOf('unsigned') !== -1;
                var enumVals = null;
                var type = row['data_type'].replace(/\s+unsigned/, '');
                var size = row['character_maximum_length'];
