...
var Schema = function() {
    ...
    return {
            ...
            decaf.each(schema.fields, function(field) {
                if (!field.reserved && !field.clientOnly) {
                    if (field.autoIncrement) {
                        ...
                        query.push('	' + quoteName(field.name) + ' ' + field.type + ' auto_increment,');
                    }
        ...
        getFromTable : function(name) {
             ...
             */
            var rows = SQL.getDataRows('select column_name, data_type, character_maximum_length from INFORMATION_SCHEMA.COLUMNS where table_name = ' + quoteName(name));
